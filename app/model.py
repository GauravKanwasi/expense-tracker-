...
class Category(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(100), nullable=False)
    user_id = Column(Integer, ForeignKey("users.id", name="fk_categories_user"), nullable=False)
...
class Transaction(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id", name="fk_transactions_user"), nullable=False)
    category_id = Column(
        Integer, ForeignKey("categories.id", name="fk_transactions_category"), nullable=False
    )
    ...
    recurring_transaction_id = Column(
        ...
        ForeignKey(
            ...
            name="fk_transactions_recurring_transaction",
            ondelete="SET NULL",
        ),
        nullable=True,
    )
    ...
    __table_args__ = (
        ...
        UniqueConstraint("recurring_transaction_id", "date", name="uq_transactions_recurring_date"),
    )
...
class RecurringTransaction(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(
        Integer, ForeignKey("users.id", name="fk_recurring_transactions_user"), nullable=False
    )
    category_id = Column(
        Integer,
        ForeignKey("categories.id", name="fk_recurring_transactions_category"),
        nullable=False,
    )
    amount = Column(MoneyType(), nullable=False)
    type = Column(String(20), nullable=False)
    ...
    __table_args__ = (Index("ix_recurring_transactions_user_due", "user_id", "next_due_at"),)
...
class Budget(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id", name="fk_budgets_user"), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "year", "month", name="uq_budgets_user_year_month"),
