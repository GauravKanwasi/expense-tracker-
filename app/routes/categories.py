...
def create_category(
    ...
    existing_category = (
        db.query(Category)
        .filter(
            func.lower(Category.name) == category.name.lower(), Category.user_id == current_user.id
        )
        .first()
    )
...
@router.get("/", response_model=list[CategoryResponse], summary="List the user's categories")
...
def delete_category(
    ...
    if transactions_using_category or recurring_using_category:
        raise HTTPException(
            status_code=400,
            detail="Cannot delete a category that has transactions or recurring schedules",
        )
