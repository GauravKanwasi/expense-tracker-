...
def create_transaction(
    ...
    category = (
        db.query(Category)
        .filter(Category.id == transaction.category_id, Category.user_id == current_user.id)
        .first()
    )
    ...
    new_transaction = Transaction(
        ...
        interest_amount=transaction.interest_amount,
        investment_action=transaction.investment_action.value
        if transaction.investment_action
        else None,
...
@router.get("/", response_model=TransactionPageResponse, summary="List transactions")
def get_transactions(
    type: str | None = Query(
        default=None, description="Optional filter: income, expense, debt, or investment."
    ),
    category_id: int | None = Query(default=None, gt=0, description="Optional category ID filter."),
    start_date: date | None = Query(
        default=None, description="Include transactions from this date."
    ),
    end_date: date | None = Query(
        default=None, description="Include transactions through this date."
    ),
    skip: int = Query(default=0, ge=0, description="Number of transactions to skip."),
    limit: int = Query(
        default=100, ge=1, le=100, description="Maximum number of transactions to return."
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    if type is not None:
        if type not in ["income", "expense", "debt", "investment"]:
            raise HTTPException(
                status_code=400, detail="Type must be income, expense, debt, or investment"
            )
...
def update_transaction(
    ...
    category = (
        db.query(Category)
        .filter(Category.id == transaction_data.category_id, Category.user_id == current_user.id)
        .first()
    )
    ...
    transaction.debt_direction = (
        transaction_data.debt_direction.value if transaction_data.debt_direction else None
    )
    transaction.interest_amount = transaction_data.interest_amount
    transaction.investment_action = (
        transaction_data.investment_action.value if transaction_data.investment_action else None
    )
...
@router.delete("/{transaction_id}", response_model=MessageResponse, summary="Delete a transaction")
